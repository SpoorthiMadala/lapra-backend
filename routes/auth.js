...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    // ✅ Step 2: Check if limit is already reached BEFORE verifying
    const maxUsers = parseInt(process.env.MAX_USERS || '50');
    const verifiedCount = await User.countDocuments({ isVerified: true });

    // ❌ Step 3: If limit already reached, reject and delete this user
    ...
    // ✅ Step 4: Mark user as verified (they made it in time!)
